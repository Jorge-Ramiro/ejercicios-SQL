...
WHERE estado LIKE '%Entregado%' 
  AND MONTH(fecha_pedido) = '01';
/*13.-Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordene el resultado de mayor a menor.*/
SELECT *
FROM pago 
WHERE forma_pago LIKE '%PayPal%' 
...
SELECT
    ...
    CONCAT(empleado.nombre, ' ', empleado.apellido1, ' ', empleado.apellido2) AS representante_ventas
FROM cliente
INNER JOIN empleado
...
INNER JOIN empleado
    ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado
WHERE cliente.codigo_cliente NOT IN (SELECT DISTINCT pago.codigo_cliente FROM pago);
...
SELECT
    IFNULL(oficina.linea_direccion1, oficina.linea_direccion2) AS direccion
FROM oficina 
...
SELECT
	CONCAT(employee.nombre, ' ', employee.apellido1) AS employees,
	CONCAT(head.nombre, ' ', head.apellido1) AS bosses
FROM empleado AS employee
INNER JOIN empleado AS head
    ON employee.codigo_jefe = head.codigo_empleado;
/*9.-Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe de sus jefe.*/
SELECT
	CONCAT(employee.nombre, ' ', employee.apellido1) AS employees,
	CONCAT(head.nombre, ' ', head.apellido1) AS bosses,
    CONCAT(jj.nombre, ' ', jj.apellido1) AS chief
FROM empleado AS employee
INNER JOIN empleado AS head 
    ON employee.codigo_jefe = head.codigo_empleado
INNER JOIN empleado AS chief 
    ON head.codigo_jefe = chief.codigo_empleado;
...
SELECT cliente.*
FROM pago 
NATURAL RIGHT JOIN cliente 
NATURAL LEFT JOIN pedido 
WHERE pago.forma_pago IS NULL 
  AND pedido.codigo_pedido IS NULL;
...
SELECT
    empleado.*,
    CONCAT(head.nombre, ' ', head.apellido1) AS jefe_directo
FROM cliente 
RIGHT JOIN empleado  
    ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado
INNER JOIN empleado AS head 
    ON empleado.codigo_jefe = head.codigo_empleado
WHERE cliente.codigo_empleado_rep_ventas IS NULL;
...
SELECT
    producto.nombre,
    COUNT(detalle_pedido.cantidad) AS unidades_vendidas,
    SUM(detalle_pedido.precio_unidad * detalle_pedido.cantidad) AS total_facturado,
    SUM(detalle_pedido.precio_unidad * detalle_pedido.cantidad) + ROUND((SUM(detalle_pedido.precio_unidad * detalle_pedido.cantidad) * .21), 2) AS total_facturado_con_impuestos
FROM detalle_pedido 
...
SELECT
    ...
    nombre_contacto
FROM cliente
INNER JOIN maximo_credito
	ON cliente.limite_credito = maximo_credito.credito_maximo;
/*2.-Devuelve el nombre del producto que tenga el precio de venta más caro.*/
WITH maximo_precio AS (
    SELECT
        MAX(precio_venta) AS precio_maximo
    FROM producto
)
...
SELECT
    nombre
FROM producto
INNER JOIN maximo_precio
    ON producto.precio_venta = maximo_precio.precio_maximo;
...
WITH maximo_stock AS (
    SELECT
		MAX(cantidad_en_stock) AS stock_maximo
	FROM producto
)

SELECT *
FROM producto
INNER JOIN maximo_stock
	ON producto.cantidad_en_stock = maximo_stock.stock_maximo;
/*6.-Devuelve el producto que menos unidades tiene en stock.*/
WITH minimo_stock AS (
    SELECT
		MIN(cantidad_en_stock) AS stock_minimo
	FROM producto
)

SELECT *
FROM producto
INNER JOIN minimo_stock
    ON producto.cantidad_en_stock = minimo_stock.stock_minimo;
/*7.-Devuelve el nombre, los apellidos y el email de los empleados que están a cargo de Alberto Soria.*/
WITH empleados_a_cargo AS (
    SELECT
        codigo_empleado
    FROM empleado
    WHERE CONCAT(nombre, ' ', apellido1) LIKE '%Alberto Soria%'
)
...
SELECT
    ...
    email
FROM empleado
INNER JOIN empleados_a_cargo
    ON empleado.codigo_jefe = empleados_a_cargo.codigo_empleado;
/*--------------- SUBCONSULTAS CON ALL Y ANY ---------------*/

/*8.-Devuelve el nombre del cliente con mayor límite de crédito.*/
WITH cliente_mayor_credito AS (
    SELECT
...
SELECT *
FROM cliente
INNER JOIN cliente_mayor_credito
    ON cliente.limite_credito = cliente_mayor_credito.maximo_credito;
...
SELECT
    ...
    (SELECT 
        COUNT(*) 
    FROM pedido 
    WHERE cliente.codigo_cliente = pedido.codigo_cliente) AS n_pedidos
FROM cliente;
...
SELECT
    cliente.nombre_cliente,
    IFNULL((SELECT SUM(total) FROM pago WHERE cliente.codigo_cliente = pago.codigo_cliente), 'No tiene pagos realizados') AS total_pagado
FROM cliente;
...
SELECT DISTINCT
    (SELECT 
        nombre_cliente 
    FROM cliente 
    WHERE cliente.codigo_cliente = pedido.codigo_cliente) AS nombre_c
FROM pedido
...
SELECT
    ...
    FROM empleado 
    WHERE cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado) AS nombre_rep,
    (SELECT 
        (SELECT 
            telefono 
        FROM oficina 
    ...
    FROM empleado 
    WHERE empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas) AS numero_tel
FROM cliente
WHERE (
    SELECT DISTINCT 
        codigo_cliente 
...
SELECT
    ...
    (SELECT 
        ...
        WHERE oficina.codigo_oficina = empleado.codigo_oficina) 
    FROM empleado 
    WHERE empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas) AS ciudad_oficina
FROM cliente;
...
SELECT
    ...
    (SELECT 
        telefono 
    FROM oficina 
    WHERE oficina.codigo_oficina = empleado.codigo_oficina) AS numero_tel
FROM empleado
WHERE (
    SELECT DISTINCT 
        codigo_empleado_rep_ventas 
    FROM cliente 
    WHERE empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas) IS NULL;
...
SELECT
    ...
    (SELECT 
        COUNT(*) 
    FROM empleado 
    WHERE oficina.codigo_oficina = empleado.codigo_oficina) AS total_empleados
FROM oficina;